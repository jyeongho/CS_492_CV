...
__all__ = ['ContrastiveLoss', 'ContrastiveArcLoss']
...
class ContrastiveArcLoss():
    def __init__(self, temperature, consistency_epoch, batchsize_label, batchsize_unlabel, out_dim, s=30.0, m=0.50, use_cosine_similarity=True):
        self.temperature = temperature
        self.use_cosine_similarity = use_cosine_similarity
        self.consistency_epoch = consistency_epoch
        self.batch_size = batchsize_label + batchsize_unlabel
    ...
    def __call__(self, z_xi, z_xj, target_x, z_ui, z_uj, arc_pred_xi, arc_pred_xj, pred_ui, pred_uj, epoch):
        #=== simclr loss ===#
        i_repr = torch.cat([z_xi, z_ui], dim=0)
        j_repr = torch.cat([z_xj, z_uj], dim=0)
        loss = self.criterion(i_repr, j_repr)
        ...
        if epoch <= self.consistency_epoch:
            ...
            #=== ArcFace Loss for labeled data ===#
            cross_entropy_criterion = nn.CrossEntropyLoss().cuda()
            pred_repr = torch.cat([arc_pred_xi, arc_pred_xj], dim=0)
            label_repr = torch.cat([target_x, target_x], dim=0)
            cross_entropy_loss = cross_entropy_criterion(pred_repr, label_repr)
            
            #==== entropy minimization for unlabeled data ===#
            B = z_ui.size()[0]
            entropy_ui = F.softmax(pred_ui, dim=1) * F.log_softmax(pred_ui, dim=1)
            entropy_ui = -1.0 * entropy_ui.sum()
            entropy_uj = F.softmax(pred_uj, dim=1) * F.log_softmax(pred_uj, dim=1)
            entropy_uj = -1.0 * entropy_uj.sum()
            entropy = (entropy_ui + entropy_uj) / (2 * B)
            return loss, cross_entropy_loss, entropy
